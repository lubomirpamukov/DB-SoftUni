...
ADD FOREIGN KEY ([AddressId]) REFERENCES [Addresses]([Id]) ON DELETE CASCADE;

ALTER TABLE [Boardgames]
ADD FOREIGN KEY ([PublisherId]) REFERENCES [Publishers]([Id]) ON DELETE CASCADE;

ALTER TABLE [CreatorsBoardgames]
ADD FOREIGN KEY ([BoardgameId]) REFERENCES [Boardgames]([Id]) ON DELETE CASCADE;

DELETE FROM [Addresses]
WHERE [Town] LIKE 'L%'


-- 5 Boardgames by Year Publication
...
SELECT
		[k].[LastName],
		CEILING([k].[AvrageRating]),
		[k].[PublisherName]
FROM 
	(SELECT 
			[c].[LastName],
			AVG([b].[Rating]) AS [AvrageRating],
