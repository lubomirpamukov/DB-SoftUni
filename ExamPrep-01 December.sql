...
ALTER TABLE [TrainsRailwayStations]
ADD FOREIGN KEY ([TrainId]) REFERENCES [Trains](Id) ON DELETE CASCADE
...
-- 8. First 3 Trains Between 08:00 and 08:59
...
FROM [Trains] AS [tr]
LEFT JOIN [Tickets] AS [ti] ON [ti].[TrainId] = [tr].[Id]
LEFT JOIN [Towns] AS [t] ON [tr].[ArrivalTownId] = [t].[Id]
WHERE CAST([tr].[HourOfDeparture] AS TIME) BETWEEN '8:00' AND '8:59'AND [ti].[Price] > 50
ORDER BY [ti].[Price] ASC


-- 9 Count of Passengers Paid More Than Average

SELECT 
		[t].[Name],
		COUNT(*) AS [PassengersCount 50/100]
FROM [Passengers] AS [p]
LEFT JOIN [Tickets] AS [tick] ON p.[Id] = [tick].[PassengerId]
LEFT JOIN [Trains] AS [tr] ON [tick].[TrainId] = [tr].[Id]
LEFT JOIN [Towns] AS [t] ON [tr].[ArrivalTownId] = [t].[Id]
WHERE ([tick].[Price]) > (SELECT AVG(Price) FROM [Tickets]) -- 76.99
...
BEGIN
	...
	SET @result	+= (SELECT COUNT(*)
					FROM Towns AS [t]
					JOIN [Trains] AS [trD] ON [trd].[DepartureTownId] = [t].[Id]
					WHERE [t].[Name] = @name)

	SET @result	+= (SELECT COUNT(*)
					FROM Towns AS [t]
					JOIN [Trains] AS [trA] ON [trA].[ArrivalTownId] = [t].[Id]
					WHERE [t].[Name] = @name)
	RETURN @result
END


-- 11. Search Passenger Travelling to Specific Town 
...
BEGIN
		...
		ORDER BY [tick].[DateOfDeparture] DESC, [p].[Name] ASC

END

EXECUTE [dbo].[usp_SearchByTown] 'Berlin'